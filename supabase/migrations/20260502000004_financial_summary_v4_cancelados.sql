-- v4: agrega métricas de cancelados (conteo, valor potencial perdido,
-- tasa cancelación). NO retira comision_referidos del payload (sigue
-- disponible para quien lo quiera consultar) pero la UI ya no la mostrará
-- porque el cliente confirmó que no tiene programa de referidos activo.
--
...
BEGIN
  ...
  cancelados AS (
    SELECT * FROM filtered
    WHERE UPPER(COALESCE(estado, '')) = 'CANCELADO'
  ),
  wallet_range AS (
  ...
  agg AS (
    SELECT
      ...
      -- Cancelados: lo que NO se facturó porque se canceló antes de despachar
...
COMMENT ON FUNCTION public.financial_summary(DATE, DATE) IS
  'Fase A — utilidad bruta operativa. v4 agrega métricas de cancelados (valor_cancelado, total_cancelados, tasa_cancelacion_pct) — informativo, NO entra en utilidad_bruta. Comision_referidos sigue en payload por compat aunque la UI ya no la muestre.';
