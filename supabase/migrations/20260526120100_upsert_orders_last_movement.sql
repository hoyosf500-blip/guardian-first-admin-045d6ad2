-- upsert_orders_from_dropi v4 — agrega last_movement_at.
--
-- Idéntica a la v3 (20260427150000_dropi_smart_upsert.sql) salvo que ahora el
-- recordset/INSERT/UPDATE manejan `last_movement_at` (el updated_at de Dropi).
-- Se agrega a la guardia IS DISTINCT FROM para que un cambio SOLO de
-- last_movement_at SÍ refresque la fila (queremos saber que se movió aunque
-- el estado textual no haya cambiado — ej. avanzó de ciudad).
--
-- Prerrequisito: 20260526120000_add_last_movement_at.sql (columna ya creada).
...
COMMENT ON FUNCTION public.upsert_orders_from_dropi(jsonb) IS
  'Bulk upsert para dropi-sync/dropi-cron con guardia IS DISTINCT FROM. v4: incluye last_movement_at (updated_at de Dropi). Filas idénticas no se reescriben → no se dispara realtime.';
