...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      -- NUEVO: confirmaciones acotadas al MISMO cohorte que total_entrantes
      -- (pedidos creados dentro del período). Así confirmados_cohorte <= total_entrantes
      -- y el aro del embudo nunca pasa de 100% cuando se trabaja backlog.
      COUNT(DISTINCT r.order_id) FILTER (
        WHERE r.module='confirmar' AND r.result='conf'
          AND EXISTS (
            SELECT 1 FROM public.orders o2
            WHERE o2.id = r.order_id
              AND o2.created_at >= v_since
...
REVOKE ALL ON FUNCTION public.operator_productivity_stats(text) FROM PUBLIC;
