-- Fix: costo_devoluciones siempre en $0 porque orders.costo_dev viene de
-- Dropi `discounted_amount` (que es un descuento aplicado, NO el costo de
-- devolución real). El costo REAL está en dropi_wallet_movements con
-- categoria='costo_devolucion' (movimientos "DEVOLUCION DE FLETE NO EFECTIVA"
-- que Dropi te debita cuando una orden no se entrega).
--
-- Esta migration actualiza el RPC para sumar la wallet en lugar de
-- orders.costo_dev. Los demás KPIs no cambian.
--
...
BEGIN
  ...
  -- FUENTE DE VERDAD para costo de devolución: la wallet de Dropi.
  -- mapCategoria en dropi-wallet-sync etiqueta como 'costo_devolucion'
  -- los movimientos cuyo código contiene "DEVOLUCION DE FLETE NO EFECTIVA".
  -- Son débitos (tipo='SALIDA'), por eso usamos ABS(monto) para el costo
  -- absoluto a restar de la utilidad y evitar ambigüedad de signo.
  wallet_range AS (
  ...
  agg AS (
    SELECT
      ...
      -- Cambio clave: sumamos la wallet, no orders.costo_dev.
      COALESCE((
        SELECT SUM(ABS(monto))
        FROM wallet_range
        WHERE categoria = 'costo_devolucion'
      ), 0) AS costo_devoluciones,
...
COMMENT ON FUNCTION public.financial_summary(DATE, DATE) IS
  'Fase A del módulo financiero — utilidad bruta. costo_devoluciones viene de la wallet (categoria=costo_devolucion), NO de orders.costo_dev (que es discounted_amount, siempre 0).';
