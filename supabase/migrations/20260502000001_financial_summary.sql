-- Fase A del módulo financiero — utilidad bruta de operación (sin gasto pauta).
--
-- Cliente confirmó que orders.costo_prod (suma de supplier_price de Dropi) ES
-- su COGS real. Cuando se conecte Meta Ads en Fase B, sumaremos al cálculo.
--
-- Patrón idéntico a logistics_summary (20260427130000): SECURITY DEFINER,
-- admin gate via public.has_role, GRANT a authenticated. Maneja `fecha`
-- como TEXT (puede venir malformada) con regex de validación antes del
-- cast a date — copiamos el guard de logistics_rpcs para no romper la
-- query si una fila tiene fecha 'garbage' o '2026-13-01'.
--
-- dropi_wallet_movements.fecha es TIMESTAMPTZ (no TEXT), así que ahí no
-- hace falta regex.
--
...
BEGIN
  ...
  WITH
  -- Pedidos en el rango (basado en `fecha` — TEXT con regex de validación,
  -- mismo patrón que logistics_rpcs).
  filtered AS (
    SELECT *
    FROM public.orders
  ...
  -- Solo entregados son ingreso real
  entregados AS (
    SELECT * FROM filtered
    WHERE UPPER(COALESCE(estado, '')) = 'ENTREGADO'
  ),
  -- Devoluciones impactan flete + costo_dev
  devueltos AS (
    SELECT * FROM filtered
    WHERE UPPER(COALESCE(estado, '')) IN
      ('DEVOLUCION', 'DEVOLUCION EN TRANSITO', 'RECHAZADO')
  ),
  -- Wallet en el rango (comisiones, retiros, etc.) — fecha ya es TIMESTAMPTZ
  wallet_range AS (
  ...
  SELECT jsonb_build_object(
    ...
    -- Utilidad bruta = ingresos - cogs - flete_entregadas - flete_devoluciones - costo_devoluciones
    -- (NO incluye gasto pauta — Fase B)
    'utilidad_bruta',
      a.ingresos_brutos
      - a.cogs
      - a.flete_entregadas
...
COMMENT ON FUNCTION public.financial_summary(DATE, DATE) IS
  'Fase A del módulo financiero — utilidad bruta de operación. NO incluye gasto pauta Meta/TikTok (Fase B).';
