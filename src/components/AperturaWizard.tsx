...
export default function AperturaWizard({ onComplete }: Props) {
    ...
    return (
      <div className="bg-card border border-border rounded-lg p-4 mb-4 border-l-[3px] border-l-cyan">
        <div className="flex items-center justify-between mb-3">
          <span className="text-sm font-bold">📋 Reporte de Apertura</span>
          <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-green/10 text-green">✅ Enviada</span>
        </div>
  ...
  const steps = [
    { emoji: '📦', q: '¿Cuántos pedidos nuevos hoy?', value: nuevos, set: setNuevos },
    { emoji: '🏷️', q: '¿Cuántas guías generadas?', value: guias, set: setGuias },
    { emoji: '📋', q: '¿Cuántos pendientes de ayer?', value: pendientes, set: setPendientes },
  ];
      ...
      {step < 3 && (
        <div className="text-center py-5">
          ...
          <div className="flex gap-2 mt-4 justify-center">
            {step > 0 && (
              <button onClick={() => setStep(step - 1)} className="flex-1 max-w-[140px] py-3 rounded-lg bg-muted text-muted-foreground font-semibold text-sm">← Atrás</button>
            )}
            {step < 2 ? (
              <button onClick={() => setStep(step + 1)} className="flex-[2] max-w-[200px] py-3 rounded-lg bg-gradient-to-r from-cyan to-blue font-semibold text-primary-foreground text-sm">Siguiente →</button>
            ) : (
              <button onClick={enviar} className="flex-[2] max-w-[200px] py-3 rounded-lg bg-green/15 text-green border border-green/25 font-bold text-sm">✅ Enviar Apertura</button>
            )}
          </div>
        </div>
      )}
