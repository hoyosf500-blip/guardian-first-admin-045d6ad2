...
/** Helper: pedido Shopify con edad en minutos y teléfono. */
...
describe("selectAutoPushCandidates", () => {
  ...
  it("salta el pedido si el teléfono YA está en Dropi (no duplica)", () => {
    const dropi = new Set(["999593016"]);
    const out = selectAutoPushCandidates([ord("A", 45, "999593016")], dropi, new Map(), baseOpts());
    expect(out).toHaveLength(0);
  });
