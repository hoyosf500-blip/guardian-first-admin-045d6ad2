...
- **Audio (notas de voz) + media sin texto — el bot NO se queda mudo (2026-06-26):** el filtro viejo `m.body` descartaba todo mensaje sin texto → una nota de voz no llegaba ni al inbox ni al bot. Ahora el filtro deja pasar `(m.body || m.media)`. En el loop de `wa-webhook`: si es AUDIO (`isAudioKind`), descarga el binario con `transport.fetchMediaBase64(id)` (Evolution: `POST /chat/getBase64FromMediaMessage/{instance}`) y lo TRANSCRIBE con OpenAI Whisper (`_shared/waTranscribe.ts`, secreto **`OPENAI_API_KEY`**, modelo `WA_STT_MODEL`=`whisper-1`); el texto queda como `body` (`🎧 …`) → la asesora lo lee y el bot razona sobre él. Otros media (foto/archivo/ubicación) reciben un marcador legible (`_shared/waMedia.ts` `mediaMarker`). Si la transcripción falla, cae al marcador y el bot igual responde. **Claude/kie.ai NO transcribe audio** — por eso el STT aparte. Visión de imágenes (que Claude LEA la foto del comprobante) queda como fast-follow.
